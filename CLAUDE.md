...
Argonaut is a web-based chat interface that lets users manage ArgoCD applications using natural language. Users talk to an AI (Claude, OpenAI, or Gemini) which translates requests into ArgoCD API calls. Think of it as a conversational DevOps assistant for Kubernetes deployments.
...
npm run db:seed      # Seed DB (creates admin/admin user)
...
│       ├── providers/route.ts      # AI Provider CRUD API
│       ├── argo-servers/route.ts   # ArgoCD Server CRUD API
...
- **AIProvider**: id, name, provider (unique: claude|openai|gemini), apiKey (encrypted), defaultModel, isDefault
- **ArgoServer**: id, name, url, authType (token|userpass), token/username/password (encrypted), insecure, isDefault
